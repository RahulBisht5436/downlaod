...
model2=tf.keras.models.load_model('identify.h5')

app = Flask(__name__)

@app.route('/',methods=["GET","POST"])
def libus():
    return render_template('libus.html')

@app.route('/fishstore')
def fishstore():
    return render_template('fishstore.html')
...
def home():
    if request.method=="POST":
        ...
        if len(result)==0:
            ...
            return render_template('home.html',name='Incorrect password or name')
        else:
     ...
     if  request.method =='POST':
            if request.form['name']!="" and request.form['password']!="":
                 ...
                 if regex.search(name):
                        ...
                        return render_template('registration.html',warning='user name must have letter ')
                 password=request.form['password']
                 regex1 = r"^(?=.*[A-Z])(?=.*[a-z])(?=.*\d)(?=.*[@$!%*?&])[A-Za-z\d@$!%*?&]{5,}$"
                 ...
                 if pattern.search(password):
                        ...
                        return render_template('registration.html',warning='password is not valid ')
                 ...
                 if result is not None:
                         return render_template('registration.html',warning='user name alredy exists ')
...
def upload():
    if request.method == 'POST':
        ...
        if ext not in  ['.jpg', '.png', '.gif']:
            return render_template('index.html',warning='unsupported file uploaded :jpg,png,gif allowed ')
    ...
    # resize the image
        ...
        if model2.predict(img)[0][0]==1:
             return render_template('index.html',warning=' upload a image of fish  ')
